...
class CrudAPI:  # MongoDB Model for ToDo CRUD Implementation
    ...
    def delete_data(self, data):    # Delete - (4) explained in next section
        filter = data['Filter']
        print("filter",filter)
        # print("filter",filter["_id"])
        response = self.collection.delete_one(filter)
        output = {'Status': 'Successfully Deleted' if response.deleted_count > 0 else "Document not found."}
        return output

# Achieving CRUD through API - '/crudapi'
@app.route('/books', methods=['GET'])     # Read MongoDB Document, through API and METHOD - GET
def read_data():
    data = request.json
...
@app.route('/books', methods=['POST'])    # Create MongoDB Document, through API and METHOD - POST
def create():
    data = request.json
...
@app.route('/books', methods=['PUT'])     # Update MongoDB Document, through API and METHOD - PUT
def update():
    data = request.json
    if data is None or data == {} or 'Filter' not in data:
        return Response(response=json.dumps({"Error": "Please provide connection information"}),
                        status=400, mimetype='application/json')
    if '_id' in data["Filter"]:
...
@app.route('/books', methods=['DELETE'])   # Delete MongoDB Document, through API and METHOD - DELETE
def delete():
    data = request.json
    if data is None or data == {} or 'Filter' not in data:
        return Response(response=json.dumps({"Error": "Please provide connection information"}),
